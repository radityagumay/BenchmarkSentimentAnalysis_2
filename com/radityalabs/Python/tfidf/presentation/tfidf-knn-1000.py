...
def preprocessing(sentences):
    ...
    for s in sentences:
        sentence = s.split(',')
        tokens = word_tokenize(sentence[0], language="english")
        new_sentence = ""
        for token in tokens:
            if len(token) > 3:
                if not token.isdigit():
                    ...
                    if is_string_not_empty(t):
                        ...
                        if valid:
                            new_sentence += t + " "
        documents.append((new_sentence, sentence[1], sentence[2]))
    return documents

def S_term_frequency(term, tokenized_document):
    return tokenized_document.count(term)

def S_save_tfidf_782_documents(documents):
    with open(path + "/Python/tfidf/presentation/tfidf-of-782-documents.pickle", "wb") as handle:
        cPickle.dump(documents, handle)

def S_save_tfidf_782_vector(documents):
    with open(path + "/Python/tfidf/presentation/tfidf-of-782-vector-documents.pickle", "wb") as handle:
        cPickle.dump(documents, handle)
...
def S_inverse_document_frequencies(tokenized_documents):
    ...
    S_save_tfidf_782_vector(all_tokens_set)

    print(all_tokens_set, len(all_tokens_set))
    return sorted_idf_values, all_tokens_set

def S_tfidf(documents):
    tokenized_documents = []
    index = 0
    for document in documents:
        tokens = word_tokenize(document[0])
...
def clean():
    data = Data()
    documents = data.load_preprocessing_documents()
    # random collection
    random.shuffle(documents)
    # pick few datas
    doc_range = []
    for index in range(0, (783 - 1)):
        doc_range.append(documents[index])
    # tf-idf
    tfidf_documents, tfidf_documents_with_term = S_tfidf(doc_range)
    S_save_tfidf_782_documents(tfidf_documents)

    print(tfidf_documents, len(tfidf_documents))
